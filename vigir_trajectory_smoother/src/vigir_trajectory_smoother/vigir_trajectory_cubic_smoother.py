...
class VigirTrajectoryCubicSmoother(object):
    ...
    def shutdown():
        print "Shutting down the VigirTrajectoryCubicSmoother!"
    ...
    def time_refactor(self, new_trajectory, num_segments):
      try:
        ...
        # Now calculate the spline parameters assuming a cubic spline for each segment
        violation = True
        count = 0
        while violation:
            count += 1
            ...
            for seg in range(0, num_segments):
                ...
                segment_violation = self.update_segment(seg, new_trajectory.points[seg], new_trajectory.points[seg+1], dT_new)
    ...
    def update_segment(self, seg, p0, p1, dT_new):
      try:
        #print "Creating quintic splines for two segment transition..."

        # Update the time
        p1.time_from_start = p0.time_from_start + rospy.Duration(dT_new[seg])

        # Calculate quartic splines for the prior segment
        #print "seg=",seg,"  dT_new = ",dT_new

        Mi = self.create_M_cubic_segment(dT_new[seg]) # dT is same for all joints in segment
        spline_list = []
        ...
        for jnt in range(len(p0.positions)):
            ...
            x = self.create_x_vector_cubic_segment(p0.positions[jnt], p0.velocities[jnt], p0.accelerations[jnt], p1.positions[jnt], p1.velocities[jnt])
            jnt_spline = np.dot(Mi,x)

            # Update the velocity value from cubic spline data
            vel = np.zeros(4); vel[0] = 3.0;  vel[1] = 2.0; vel[2] = 1.0;
            p_vel  = (jnt_spline*vel)[:-1]  # quadratic
            vel_seg = np.polyval(p_vel,dT_new[seg])
            p1.velocities[jnt] = deepcopy(vel_seg)

            # Update the acceleration value from cubic spline data
            acc  = np.zeros(4); acc[0] =6.0;  acc[1] = 2.0;
            p_acc  = (jnt_spline*acc)[:-2]  # linear
            acc_seg = np.polyval(p_acc,dT_new[seg])
    ...
    """
    def create_M_cubic_segment(self, dT): # dT is same for all joints in segment
        ...
        M[3][0] =   pow(dT,3); M[3][1] = pow(dT,2); M[3][2] = dT; M[3][3] = 1.0 # q[1]
    ...
    def create_x_vector_cubic_segment(self, q0, dq0, ddq0, q1, dq1):
        unknowns = 4
